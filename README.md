...
| Large wildcard runs | `CACHE_BRONZE`, `PREFLIGHT_KIND_COUNTS`, `BATCH_METADATA_WRITES`, `METADATA_FLUSH_INTERVAL`, `SCHEMA_PREFLIGHT`, `SCHEMA_FETCH_PARALLELISM`, `WIDE_MAX_CARDINALITY_CAP` | Keep defaults for broad runs to reduce repeated bronze scans, parallelize bounded schema lookups, limit small Delta commits, and cap wide-mode array expansion. |
