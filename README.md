...
| Reassembled table | One wide table per kind when `REASSEMBLE = True`. Struct arrays are expanded into indexed columns, tags are pivoted, and primitive arrays are concatenated. | `welllog` |
...
7. Set `RUN_PROFILE = "full"` when the configuration is correct.
8. Run the notebook sections in order through Results Summary.
9. Review the generated Silver Layer tables and the `silver_run_info` table.
10. Connect downstream analytics, reporting, or data engineering workloads to the Silver Layer outputs.
...
| `INCREMENTAL` | `ADME_INCREMENTAL` | `False` | Use incremental writes when `true`; otherwise overwrite output tables. |
| `LIMIT` | `ADME_LIMIT` | `0` | Maximum records per kind. `0` means no limit. |
| `REASSEMBLE` | `ADME_REASSEMBLE` | `False` | Create one wide table per kind instead of parent and child tables. |
...
| `workspace_id` | `ADME_WORKSPACE_ID` | Runtime value or notebook fallback | Fabric workspace identifier. |
| `lakehouse_id` | `ADME_LAKEHOUSE_ID` | Runtime value or notebook fallback | Fabric lakehouse identifier. |
| Lakehouse name fallback | `ADME_LAKEHOUSE_NAME` | `osducatalog` | Used only when the notebook must build a OneLake path without an attached lakehouse identifier. |
...
## Choose decomposition or reassembly

Use the output mode that matches the table shape required by downstream consumers.

| Mode | Set | Creates | Use when |
| --- | --- | --- | --- |
| Decomposition | `REASSEMBLE = False` | Parent table plus child tables for arrays. | You want a normalized relational Silver Layer model with separate tables for repeated fields. |
| Reassembly | `REASSEMBLE = True` | One wide table per kind. | You need a single table per OSDU kind for BI, export, simplified SQL, or other tools that prefer denormalized data. |
...
- Generated child tables when `REASSEMBLE = False`.
...
- Use incremental mode only when the bronze source and downstream consumers can tolerate upsert behavior for parent tables and child table replacement for changed IDs.
...
| Output table shape is too normalized | Use `REASSEMBLE = True` to create one wide table per selected kind. |
